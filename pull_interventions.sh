...
q "select json_build_object(
 ...
 'effort', json_build_object('campus_own_h',(select round((sum(minutes_spent)/60.0)::numeric,1) from intervention_requests where section='campus_dri' and entry_kind='manual_work' and minutes_spent>0),'campus_own_n',(select count(*) from intervention_requests where section='campus_dri' and entry_kind='manual_work' and minutes_spent>0),'curriculum_answer_h',(select round((sum(completion_minutes)/60.0)::numeric,1) from intervention_requests where completion_minutes>0),'curriculum_answer_n',(select count(*) from intervention_requests where completion_minutes>0),'cm_eligible',(select count(*) from intervention_requests where section='campus_dri' and entry_kind='intervention_request' and completed and created_at>='2026-09-11'),'cm_have',(select count(*) from intervention_requests where section='campus_dri' and entry_kind='intervention_request' and completed and created_at>='2026-09-11' and completion_minutes is not null))
